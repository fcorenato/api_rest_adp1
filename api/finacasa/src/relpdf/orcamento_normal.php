...
$pdf->SetCreator(utf8_decode('BIV'));
...
//se boleto santander
if ($dados['cond_pgto'] == 28) {
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(20, $altura_linha, utf8_decode(' '), '0', 0, 'L', $colore);
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(40, $altura_linha, utf8_decode(' '), '0', 0, 'L', $colore);
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(25, $altura_linha, utf8_decode('Primeira Parcela: '), '0', 0, 'R', $colore);
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(60, $altura_linha, utf8_decode($dados['bolstd_prim_parc']) . ' dias', '0', 0, 'L', $colore);
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(20, $altura_linha, utf8_decode('Qtde Parcelas: '), '0', 0, 'R', $colore);
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(25, $altura_linha, utf8_decode($dados['bolsdt_qtd_parc']), '0', 0, 'L', $colore);
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Ln(); //quebra linha
}
    ...
    while ($dadosi = mysql_fetch_array($pesquisa2)) {
        ...
        if ($dadosi["permitePalletAberto"] == 'S' and $dadosi["app_tx_pal"] == 'S') {
            if ($dadosi["taxaPalletAbertoR"] <> 0) {
                ...
                $dadosi["prc_tab"] = $dadosi["prc_tab"] + ($dadosi["taxaPalletAberto"] / $dadosi["qtde"]);
