...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
	...
	$result_query2 = mssql_query($query2);
	$qtde_query2 = mssql_num_rows($result_query2);

	if ($qtde_query2 > 0) {
		while ($campos = mssql_fetch_array($result_query2)) {
	...
	$result_query3 = mssql_query($query3);
	$qtde_query3 = mssql_num_rows($result_query3);

	if ($qtde_query3 == 0) {
		$select_cond_pgto .= '';
	} else {
		while ($campos = mssql_fetch_array($result_query3)) {
