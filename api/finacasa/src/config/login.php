...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    un.codigo as un_codigo, un.descricao, un.cod_protheus, un.tabelas, un.orcamentos, un.armazens, un.cond_pgto_lib, un.split_pgto, un.fabricantes, un.bling_apikey  FROM `sys_usuarios` as u
    ...
    if ($linhas == 0) {
        ...
        while ($dados = mysql_fetch_array($pesquisa)) {
            ...
            $un_bling_apikey = $dados["bling_apikey"];
        ...
        if (($status == 'A') and ($usuario != $senha)) {
            if ($senha != $post_pwd) {
                ...
                $_SESSION["un_bling_apikey"] = $un_bling_apikey;
