...
if ($resultado->error) {
    ...
    //atualizando token no banco de dados
    echo 'atualizando token no banco de dados Finacasa...<hr>';
    include('../../config/conexao.php');   
    $atu_pv_ibge = mysql_query("UPDATE sys_unidades SET bling_access_token='$tk', bling_refresh_token='$tk_refresh', bling_token_update = '$time_update'  WHERE codigo = 1") or die(mysql_error());
    
    //atualizando token no banco de dados da finacasa cores
    echo 'atualizando token no banco de dados Finacasa Cores...<hr>';
    include('../../config/conexao_finacasacores.php');
    $atu_pv_ibge = mysql_query("UPDATE sys_unidades SET bling_access_token='$tk', bling_refresh_token='$tk_refresh', bling_token_update = '$time_update'  WHERE codigo = 1") or die(mysql_error());
}
