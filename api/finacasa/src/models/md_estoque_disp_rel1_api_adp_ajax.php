...
include_once('../api/bling_produtos_get.php');
//gerado array: produtos_array
// echo 'api produtos ok<hr>';

//totalizando estoque por item:
// include_once('../api/bling_prod_estoque_get.php');
...
if ($qtde_query3 > 0) {
    while ($dados_est = mysql_fetch_array($result_query3)) {
        $est_array_qtde_it[$dados_est['referencia']] += $dados_est['saldo_disp'];
    }
}


//totalizando ops por item:
include_once('../api/bling_op_get.php');
...
include_once('../api/bling_pedido_compras_get.php');
...
include_once('../api/bling_pedido_vendas_get.php');
...
// $conteudo = mysql_real_escape_string($relatorio_result);
$conteudo = str_replace('"', 'ASPD', $relatorio_result);
