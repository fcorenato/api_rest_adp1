<?php 
	 //funcao para mostra quantos domingos tem no mes
	 function domingosNoMes( $mes, $ano ) {
	    $t = gmmktime( 0, 0, 0, $mes, 1, $ano );
	    $ult = date( 't', $t );
	    $sem  = date( 'w', $t );
	    return floor( ( $ult - 1 ) / 7 ) + ( $sem == 0 || $sem > ( 35 - $ult ) );
	}

	//funcao para mostra valor em formato 1.023,02
	function exibirValor( $valor ) {
	    $nValor = number_format($valor,    2, ',', '.');
	    return $nValor;
	}

	//funcao remove acentos
	function remove_accents($string) {
		$caracteres_sem_acento = array(
			'Š'=>'S', 'š'=>'s', 'Ð'=>'Dj','Â'=>'Z', 'Â'=>'z', 'À'=>'A', 'Á'=>'A', 'Â'=>'A', 'Ã'=>'A', 'Ä'=>'A',
			'Å'=>'A', 'Æ'=>'A', 'Ç'=>'C', 'È'=>'E', 'É'=>'E', 'Ê'=>'E', 'Ë'=>'E', 'Ì'=>'I', 'Í'=>'I', 'Î'=>'I',
			'Ï'=>'I', 'Ñ'=>'N', 'Å'=>'N', 'Ò'=>'O', 'Ó'=>'O', 'Ô'=>'O', 'Õ'=>'O', 'Ö'=>'O', 'Ø'=>'O', 'Ù'=>'U', 'Ú'=>'U',
			'Û'=>'U', 'Ü'=>'U', 'Ý'=>'Y', 'Þ'=>'B', 'ß'=>'Ss','à'=>'a', 'á'=>'a', 'â'=>'a', 'ã'=>'a', 'ä'=>'a',
			'å'=>'a', 'æ'=>'a', 'ç'=>'c', 'è'=>'e', 'é'=>'e', 'ê'=>'e', 'ë'=>'e', 'ì'=>'i', 'í'=>'i', 'î'=>'i',
			'ï'=>'i', 'ð'=>'o', 'ñ'=>'n', 'Å'=>'n', 'ò'=>'o', 'ó'=>'o', 'ô'=>'o', 'õ'=>'o', 'ö'=>'o', 'ø'=>'o', 'ù'=>'u',
			'ú'=>'u', 'û'=>'u', 'ü'=>'u', 'ý'=>'y', 'ý'=>'y', 'þ'=>'b', 'ÿ'=>'y', 'ƒ'=>'f',
			'Ä'=>'a', 'î'=>'i', 'â'=>'a', 'È'=>'s', 'È'=>'t', 'Ä'=>'A', 'Î'=>'I', 'Â'=>'A', 'È'=>'S', 'È'=>'T', '°'=>'', 'º'=>'', 'ª'=>'', ''=>'',
		);

		//remove acento e deixa maiscula
		$nova_string = strtoupper(strtr($string, $caracteres_sem_acento));
	
		return $nova_string;
	}
		
 ?>