...
if ($api_bling_status == 'on') {
    ...
    $result = mysql_query("INSERT INTO md_pedidos_bling_callback (referencia, descricao, un, deposito, saldo, json) VALUES ('api-start', 'iniciou api - $timestart', 'un', 'Manual','0', 'start')") or die(mysql_error());
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        //salvando o json recebido pra analise
         ...
         //$remove_ref_saldo_antigo = mysql_query("DELETE FROM `md_pedidos_bling_callback` WHERE deposito = 'manual'") or die(mysql_error());


        $json_receive_at = date("Y-m-d H:i:s");
        $result = mysql_query("INSERT INTO md_pedidos_bling_callback (referencia, descricao, un, deposito, saldo, json) VALUES ('api', 'json recebido - $json_receive_at', 'm2', 'Manual','0', '$json_params')") or die(mysql_error());
        ...
        if (strlen($json_params) > 0 && isValidJSON($json_params)) {
            ...
            //removendo ref com saldo antigo, caso exista.
            //$remove_ref_saldo_antigo = mysql_query("DELETE FROM `md_pedidos_bling_callback` WHERE referencia = '$ref_prod'") or die(mysql_error());
            ...
            foreach ($dados->retorno->estoques[0]->estoque->depositos as $dep) {
                ...
                if ($deposito_id == '11919578899') {
                    $result = mysql_query("INSERT INTO md_pedidos_bling_callback (referencia, descricao, un, deposito_id, deposito, saldo, saldo_disp) VALUES ('$ref_prod', '$ref_desc', ' ', '$deposito_id', '$deposito_nome', '$saldo','$saldo_disp')") or die(mysql_error());
                }
                
            }
        } else {
            $dados = file_get_contents("php://input");
            $result = mysql_query("INSERT INTO md_pedidos_bling_callback (referencia, descricao, un, deposito, saldo, json) VALUES ('api', 'ERRO! - json recebido - $json_receive_at', 'm2', 'Manual','0', '$json_params')") or die(mysql_error());
        }
    } // fim do POST
}
