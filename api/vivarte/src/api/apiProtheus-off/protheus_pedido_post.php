...
if ($linhas_pv == 0) {
    ...
    //Tratando razao e nome fantasia
    $dadospv['cliente_razao'] = substr($dadospv['cliente_razao'], 0, 60);
    $dadospv['cliente_razao'] = remove_accents($dadospv['cliente_razao']);

    if ($dadospv['cliente_nomef'] == '') {
        $dadospv['cliente_nomef'] = substr($dadospv['cliente_razao'], 0, 30);
    } else {
        $dadospv['cliente_nomef'] = substr($dadospv['cliente_nomef'], 0, 30);
    }
    $dadospv['cliente_nomef'] = remove_accents($dadospv['cliente_nomef']);
...
// API HOMOLOGA
$url = 'http://191.37.68.150:8096/rest/WSPEDVEN';
