...
if ($linhas_pv == 0) {
    ...
    $pv_undv = mysql_query("SELECT un.bling_vend_id, un.bling_vend_id2 FROM `sys_unidades` as un
    ...
    if ($linhas_pv_undv == 0) {
        ...
        $cod_und_vend1_bling2 = $dados_pv_undv['bling_vend_id2'];
    }
...
$pesquisa_itens = mysql_query("SELECT i.*, p.id_bling_prod, p.id_bling_prod2 FROM md_vendas_pedidos_itens as i 
...
if ($linhas_itens == 0) {
    ...
    while ($dadosi = mysql_fetch_array($pesquisa_itens)) {
        if ($dadospv['empresa'] == 'VIVARTE(AG)') {
            $id_bling_prod = $dadosi["id_bling_prod2"];
            ...
            //     //SE PEDIDO FORA CEARA E NÃO CONTRIBUINTE = BLING VM
            //     $preco_tab = (float) number_format($dadosi["prc_tab"] - $dadosi["desconto"], 2, '.', '');
            // } else {
...
if ($integra_bling) {
    ...
    if ($dadospv['empresa'] == 'VIVARTE(AG)') {
        ...
        //id cliente no bling
        $id_cli_bling = $id_cli_bling_agas;

        //cod_vendedor_bling
        $bling_vend_id = $cod_und_vend1_bling2;

        //categoria venda
        $categoria = '14627473983';
    } else {
        //token VIVARTE
    ...
    if ($dadospv['qtde_parc'] > 1) {
        ...
        $valor_total = $dadospv['total_final'];
