...
$pdf->AddPage();
$pdf->SetTitle(utf8_decode('Vivarte - Orçamento'));
...
$pdf->MultiCell(190, $altura_linha, utf8_decode('1 - SERÁ EMITIDO UMA NOTA FISCAL NO VALOR DE R$ ') . number_format($total_produtos + $dados["frete_valor"], 2, ',', '.') . utf8_decode(' PELA VIVARTE E UMA NOTA FISCAL COMPLEMENTAR NO VALOR DE R$ ') . number_format($total_servicos, 2, ',', '.') . utf8_decode('.'));

if ($empresa == 'VIVARTE') {
    $pdf->MultiCell(190, $altura_linha, utf8_decode('2 - FRETE: DEVERÁ SER PAGO À VISTA, NO ATO DO FECHAMENTO DO PEDIDO, DIRETAMENTE À VIVARTE.
	DADOS PARA PAGAMENTO DO FRETE: VIVARTE REVESTIMENTOS - Banco: Santander (033) Agencia: 3132 Conta Corrente: 13011698-0 CNPJ. 06.010.153/0003-68. PIX: 06.010.153/0003-68'),1, 'L', False);
...
$pdf->SetFont('Arial', '', 6);
$pdf->MultiCell(190, $altura_linha, utf8_decode('3.	O prazo de entrega só é contado a partir da confirmação do pagamento e envio do pedido assinado pelo cliente. Sem isso, o pedido não é implantado.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('4.	Os valores desse orçamento incluem ICMS, IPI e DIFAL. Qualquer outra cobrança de taxas/impostos no destino é de responsabilidade exclusiva do cliente, ficando a vendedora isenta de quaisquer custos, inclusive de cobranças extras de atraso/estocagem pela transportadora.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('5.	Este orçamento não garante reserva de estoque.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('6.	A previsão de faturamento/entrega pode sofrer alteração a depender da disponibilidade do estoque no fechamento do pedido.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('7.	O prazo constante no orçamento refere-se ao período para liberação e despacho por parte da vendedora. O prazo de trânsito para entrega é de total responsabilidade da transportadora terceirizada contratada. Eventuais atrasos na entrega do produto, reconhecidamente provocados por terceiros e alheios ao controle da vendedora, não são passíveis de ressarcimento.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('8.	A entrega dos produtos será efetuada no endereço constante no campo endereço de entrega. Caso o endereço informado não seja o correto ou não haja pessoa disponível para recebimento na data agendada, será cobrado 50% do valor do frete a título de despesa com re-entrega que deve ser pago antecipadamente.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('9.	Em caso de necessidade de capatazia para descarregamento, esta deve ser providenciada pelo cliente.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('10.	Entrega efetuada no térreo. Em caso de necessidade de entrega em subsolo ou subir com os produtos pode ser cobrado uma taxa de dificuldade de entrega.'));
...
$pdf->MultiCell(190, $altura_linha, utf8_decode('12.	Confira a mercadoria do ato da entrega e na presença do funcionário da transportadora. Caso haja avaria, divergência ou danos RECUSE O LOTE TOTAL correspondente ao material avariado. Notifique no conhecimento de transporte e o responsável pela entrega deve assinar com ciente. NÃO SERÃO ACEITAS RECLAMAÇÕES POSTERIORES.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('13. OS PRODUTOS ESTÃO SUJEITOS A VARIAÇÃO DE CORES E TONALIDADES EM RELAÇÃO AS AMOSTRAS. No caso de pedidos complementares, não garantimos a mesma tonalidade.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('14. Todas a orientações de assentamento, manutenção e limpeza dos produtos devem ser seguidas, sob pena de perda da garantia e podem ser encontradas em nosso site: www.vivarterevestimentos.com.br.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('15. NÃO SERÃO ACEITAS RECLAMAÇÕES OU TROCAS DE PRODUTOS APLICADOS. VERIFIQUE TODO O MATERIAL ANTES DE EXECUTAR A INSTALAÇÃO NA OBRA. '));
