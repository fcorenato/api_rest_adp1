...
$pdf->AddPage();
$pdf->SetTitle(utf8_decode('BIV - Orçamento'));
$pdf->SetCreator(utf8_decode('BIV - VIVARTE'));
...
if ($qtde_itens == 0) {
    ...
    while ($dadosi = mysql_fetch_array($pesquisa2)) {
        ...
        //verificar se é PALEO para obs: Especial
        if ($dadosi['codigo'] == 'PLAT0101' or $dadosi['codigo'] == 'PLAZ0101' or $dadosi['codigo'] == 'PLCB0101') {
            $exibir_obs_palo = true;
        }
...
$pdf->Cell(30, $altura_linha, utf8_decode('Total'), 'TL', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('Desc Total'), 'TL', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('Total c/'), 'TL', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('Frete'), 'TL', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('Total Final'), 'LRT', 0, 'C', $colore);
$pdf->Ln(); //quebra linha
$pdf->Cell(20, $altura_linha, utf8_decode('Volumes'), 'LB', 0, 'C', $colore);
$pdf->Cell(20, $altura_linha, utf8_decode('KG'), 'LB', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('R$'), 'LB', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('R$'), 'LB', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('Desc R$'), 'LB', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('R$'), 'LB', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, utf8_decode('R$'), 'LRB', 0, 'C', $colore);
$pdf->Ln(); //quebra linha
$colore = False;
$altura_linha = 8;
$pdf->SetFont('Arial', 'B', 8);
...
$pdf->Cell(30, $altura_linha, number_format($dados["total_cimp"] + $dados["total_desc"], 2, ',', '.'), '1', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, number_format($dados["total_desc"], 2, ',', '.'), '1', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, number_format($dados["total_cimp"], 2, ',', '.'), '1', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, number_format($dados["frete_valor"], 2, ',', '.'), '1', 0, 'C', $colore);
$pdf->Cell(30, $altura_linha, number_format($dados["total_final"], 2, ',', '.'), '1', 0, 'C', $colore);
$pdf->Ln(); //quebra linha
...
$pdf->MultiCell(190, $altura_linha, utf8_decode('1.	O prazo de entrega só é contado a partir da confirmação do pagamento e envio do pedido assinado pelo cliente. Sem isso, o pedido não é implantado.'));
...
$pdf->MultiCell(190, $altura_linha, utf8_decode('8.	Entrega efetuada no térreo. Em caso de necessidade de entrega em subsolo ou subir com os produtos pode ser cobrado uma taxa de dificuldade de entrega.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('9.	Em situações consideradas pela transportadora como atípica (ex. Entrega em horários especiais, dificuldade de acessibilidade do ponto de descarregamento, etc...) poderá ser cobrado uma taxa extra de entrega não inclusa nesse orçamento.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('10.	Confira a mercadoria do ato da entrega e na presença do funcionário da transportadora. Caso haja avaria, divergência ou danos RECUSE O LOTE TOTAL correspondente ao material avariado. Notifique no conhecimento de transporte e o responsável pela entrega deve assinar com ciente. NÃO SERÃO ACEITAS RECLAMAÇÕES POSTERIORES.'));
...
$pdf->MultiCell(190, $altura_linha, utf8_decode('12. Por serem fabricados artesanalmente a partir de rochas naturais, passando apenas por processos de corte, o produto final preservará todas as características da rocha natural. Desta forma, as características abaixo podem ocorrer, não sendo consideradas vícios do produto:'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('          -	Variações de forma (tamanho e espessura), tonalidades e cores;'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('          -	Irregularidades na superfície, bordas e cantos;'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('          -	Presença natural de manchas e veios;'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('13. Por serem materiais que apresentam diversos graus de porosidade, após a instalação ou aplicação, poderão ainda apresentar eflorescências, oxidação ou outras variações de origem natural. Pode ainda haver acúmulo de sujeira, manchas e limo quando expostos aos elementos naturais (sol, chuva, calor, frio, umidade). Para evitar a ocorrência de tais fenômenos, deve-se proteger a superfície do produto com protetores/ impermeabilizantes adequados. Verificar o manual técnico do produto.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('14. As citadas variações e outras decorrentes da origem natural dos materiais, independente da época em que ocorram, não serão consideradas vício ou defeito do produto e não autorizarão a sua rejeição, devolução, abatimentos de preços ou quaisquer indenizações. '));
$pdf->MultiCell(190, $altura_linha, utf8_decode('15. Todas a orientações de assentamento, manutenção e limpeza dos produtos devem ser seguidas, sob pena de perda da garantia e podem ser encontradas em nosso site: www.vivarterevestimentos.com.br.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('16. NÃO SERÃO ACEITAS RECLAMAÇÕES OU TROCAS DE PRODUTOS APLICADOS. VERIFIQUE TODO O MATERIAL ANTES DE EXECUTAR A INSTALAÇÃO NA OBRA.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('17. O consumidor declara e reconhece expressamente que adquiriu o produto objeto deste contrato mediante amostra previamente apresentada e aprovada, de acordo com suas especificações e necessidades e/ou por encomenda ou sob especificação, conforme características específicas por ele próprio previamente definidas.'));
$pdf->MultiCell(190, $altura_linha, utf8_decode('18. O ato do pagamento do valor do pedido é considerado concordância tácita, por parte do cliente, com as cláusulas deste Termo de Venda.'));


if ($exibir_obs_palo) {
    $pdf->Ln(); //quebra linha
    
    $pdf->MultiCell(190, $altura_linha, utf8_decode('ATENÇÃO. Como o produto PALEO tem tamanhos variados e irregulares, a área de cobertura e rendimento do produto vai depender da técnica de assentamento e efeito visual desejado. Normalmente o assentamento é feito de duas maneiras:'));

    $pdf->MultiCell(190, $altura_linha, utf8_decode('1: Com junta e peças de todos os tamanhos: proporciona o maior rendimento. É importante que, na paginação, sejam usadas peças de todos tamanhos recebidos e não se faça ajuste de formatos quebrando-se peças para melhor encaixe, caso contrário a área de cobertura do produto será reduzida e a quantidade adquirida pode não ser suficiente. Calcular perda de 10% a 15%.'));
    $image = "obs_paleo_img1.jpg";
    $pdf->Cell(32, 38, $pdf->Image($image, $pdf->GetX(), $pdf->GetY(), 33.78), 0, 0, 'L', false);
    $pdf->Ln(); //quebra linha

    $pdf->MultiCell(190, $altura_linha, utf8_decode('2: Junta mínima e somente com peças maiores: Proporciona um visual mais uniforme, porém o rendimento é menor. Nessa técnica, é feita uma seleção das peças maiores e um ajuste nos seus formatos, quebrando/cortando os cantos para encaixar perfeitamente na paginação, deixando o menor espaço possível entre as peças. Desta forma, a cobertura do produto será reduzida. Calcular perda de 25% a 30%.'));
    $image = "obs_paleo_img2.jpg";
    $pdf->Cell(32, 38, $pdf->Image($image, $pdf->GetX(), $pdf->GetY(), 33.78), 0, 0, 'L', false);
    $pdf->Ln(); //quebra linha
    $pdf->MultiCell(190, $altura_linha, utf8_decode('3:	PRODUTO EMBALADO E TRANSPORTADO EM PALLETS, QUE SERÃO ABERTOS PARA O DESCARREGAMENTO. O PRODUTO NÃO SERÁ RE-PALETIZADO.'));
    
}
 if ($qtd_item <= 2 and $exibir_obs_palo == false) {
    $pdf->AddPage(); //quebra pagina
}

$pdf->MultiCell(190, $altura_linha, utf8_decode('Para baixar nossos manuais técnicos, aponte a câmera do celular para o QR Code abaixo:'));
$image = "qrcode_vivarte_manuais_tecnicos.jpg";
$pdf->Cell(32, 32, $pdf->Image($image, $pdf->GetX(), $pdf->GetY(), 33.78), 0, 0, 'L', false);
$pdf->Ln(); //quebra linha
...
$altura_linha = 6;
if ($empresa == 'VIVARTE') {
    $pdf->Cell(190, $altura_linha, utf8_decode(' OBS: DADOS PARA PAGAMENTO: VIVARTE REVESTIMENTOS - Banco: Santander (033) Agencia: 3132 Conta Corrente: 13011698-0 CNPJ. 06.010.153/0003-68. PIX: 06.010.153/0003-68'), '1', 0, 'L', $colore);
} else if ($empresa == 'VIVARTE(AG)') {
    $pdf->Cell(190, $altura_linha, utf8_decode(' OBS: DADOS PARA PAGAMENTO: AGAS ROCHAS ORNAMENTAIS - Banco: C6 Bank (336) Agencia: 0001 Conta Corrente: 22500368-6 CNPJ. 48.061.210/0001-16.   PIX: 48.061.210/0001-16'), '1', 0, 'L', $colore);
}
$pdf->Ln(); //quebra linha
...
//publicando
$pdf->Output('BIV Vivarte -  Orcamento ' . $id . '.pdf', 'I');
