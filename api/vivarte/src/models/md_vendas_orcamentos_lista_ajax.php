...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  } else  if ($perfil == 'R') { //representante
    $pesquisa_por_unidade = "AND un.rep_codigo = '$codigo_usuario'";
  ...
  //se unidade = 0004(loja vc) e nao for evelyne vendedor exengar apenas seus orcamentos
  if ($un_codigo_user == 4 and $codigo_usuario != 6 and $codigo_usuario != 11) {
  ...
  $pesquisa = mysql_query("SELECT DISTINCT pv.id, pv.empresa, pv.orc_created_user, pv.cliente_codigo, pv.cliente_razao, pv.filial_atend, pv.cpf_cnpj,   pv.vend1, pv.vend2 , pv.orc_created_at, pv.orc_data_emissao, pv.orc_data_valid, pv.pedido_conv_date, pv.cond_pgto, pv.total_final, pv.status, un.descricao,un.codigo, cp.forma_pgto, pv.status_pgto
  FROM md_vendas_pedidos as pv
  LEFT JOIN md_vendas_pedidos_itens as ip ON pv.id = ip.pedido_id
  LEFT JOIN md_vendas_cpgto as cp ON pv.cond_pgto = cp.codigo
  ...
  if ($linhas == 0) {
    ...
    while ($dados = mysql_fetch_array($pesquisa)) {
      ...
      if ($dados["status"] == 'A') {
        ...
        if ($dados["forma_pgto"] == 'CARTAO' and $dados["empresa"] == 'VIVARTE' and $dados["status_pgto"] != 'P') {
          $doc_acoes .= '<a class="dropdown-item bg-warning text-dark" href="md_vendas_orcamento_upd.php?id=' . $dados["id"] . '&state=view&converte=7" style="color:black !important"><i class="fas fa-comment-dollar"> Pagamento</i></a>';
        }
      ...
      //se perfil representante modificar acessos
      if ($perfil == 'R') { //representante
        $doc_acoes = '<a target="_blank" class="dropdown-item bg-dark" href="src/relpdf/orcamento.php?id=' . $dados["id"] . '&state=view"><i class="fas fa-print"> Imprimir</i></a>';
      }
