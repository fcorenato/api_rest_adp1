...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($qtde_query1 == 0) {
            ...
            $query_prc = "
            select ti.codigo_tabela, ti.referencia, ti.preco_venda, ti.desconto
            ...
            if ($qtde_query2 > 0) {
                while ($da1 = mysql_fetch_array($result_query2)) {
                    $ref_precos[trim($da1['referencia']) . '-' . $da1['codigo_tabela']] = $da1['preco_venda'] * (1-$da1['desconto']);;    
                }
            }
            ...
            foreach ($ref_encontradas as $key => $value) {
                ...
                $resultado_rel .= '
                <tr>
                    <td class="p-1"><a href="#" class="btn btn-sm btn-success w-100 consulta_estoque" prod="' . $key . '" >CONSULTAR ESTOQUE</a></td>
                     <td>' . $key . '</td>
                     <td>' . $value[0] . '</td>
                     <td>' . $value[4] . '</td>
                     <td>' . $value[1] . '</td>
                ...
                foreach ($tabelas_und as $key2 => $value2) {
                    //se tabela for 238 (tabela Vivarte zona franca revenda) ou AGAS 232 e 233
                    // nao calcula IPI 
                    if ($value2 == '238' || $value2 == '541' || $value2 == '232' || $value2 == '233' || $value2 == '532' || $value2 == '252') {
