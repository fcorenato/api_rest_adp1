...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT us.codigo, us.usuario, us.nome_completo, us.email, us.instagram_pessoal, us.telefone, us.data_niver, us.perfil, us.status, un.descricao,  un.tabelas, un.status as unid_status
    ...
    if ($linhas == 0) {
        ...
        $resultado_rel = '
              ...
              <thead>
                ...
                <th data-field="price">Instagram</th>
        ...
        while ($dados = mysql_fetch_array($pesquisa)) {
            ...
            if ($dados["data_niver"] != '') {
                $data_n = date("d/m/Y", strtotime($dados["data_niver"]));
            } else {$data_n = '';}
            ...
            $resultado_rel .= '
                ...
                <td>' . $dados["email"] . '</td>
                <td>' . $dados["instagram_pessoal"] . '</td>
                <td>' . $data_n . '</td>
                <td align="center">' . $dados["perfil"] . '</td>
                <td>' . $dados["descricao"] . ' '.$unidstatus.'</td>
                <td>' . $status.'</td>
                <td>
                    <a class="btn btn-info btn-sm" href="sys_parceiro_upd.php?codigo=' . $dados["codigo"] . '"><i class="fas fa-pen"></i></a>
                </td>
                </tr>
                ';
        }
