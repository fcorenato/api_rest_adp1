...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($un_codigo > 1) {
    $filtro_loja = "AND us.unidade_codigo = '$un_codigo'";
  }
  ...
  // pesquisar estoque disponivel
  if ($tipo_rel == 'analitico_estoque') {
    $query0 = "SELECT referencia, tipo_estq, SUM(saldo_disp) as saldo_disp_total FROM md_estoque_disponivel_detail GROUP BY referencia, tipo_estq";
    $result_query0 = mysql_query($query0);
    $qtde_query0 = mysql_num_rows($result_query0);

    if ($qtde_query0 > 0) {
      while ($dados_estq = mysql_fetch_array($result_query0)) {
        $saldo_disp_item_array[$dados_estq['referencia']] += $dados_estq['saldo_disp_total'];
      }
    }

    $ocultar_qtde_disp = '';
  } else {
    $ocultar_qtde_disp = ' style="display:none;" ';
  }
  ...
  if (isset($_POST['referencia'])) {
    ...
    if ($referencia_cod != '') {
      $pesquisa_por_ref = " and (pi.codigo LIKE '%$referencia_cod%')";
    }
  }
  ...
  $query1 = "
      ...
      left JOIN md_vendas_tabpreco tb ON (tb.codigo = pd.tabela_preco)
      where pi.status = 'A'
      $filtro_unidade
      $filtro_loja
      $pesquisa_por_ref
  ...
  if ($qtde_query1 == 0) {
    ...
    $resultado_rel = '
              ...
              <thead>
                <tr>
                  ...
                  <th>Produto</th>
                  <th>Qtde</th>
                  <th ' . $ocultar_qtde_disp . '>Etq Disp</th>
                  <th>Preço</th>
                  <th>Valor R$ c/ ipi</th>
    ...
    while ($campos = mysql_fetch_array($result_query1)) {
      ...
      //verificando saldo disponivel da refentecia
      $saldo_ref_disp = $saldo_disp_item_array[$campos['codigo']];
      if ($campos['qtde'] <= $saldo_ref_disp) {
        $bg_saldo_disp = ' style="background-color: LightGreen;" ';
      } else {
        $bg_saldo_disp = ' ';
      }

      if ($saldo_ref_disp <= 0 and $tipo_rel == 'analitico_estoque') {
        $imprime_item = false;
      }


      if (($tipo_rel == 'analitico' or $tipo_rel == 'analitico_estoque') and $imprime_item) {
        $resultado_rel .= '<tr>
            ...
            <td align="right" ' . $ocultar_qtde_disp . ' ' . $bg_saldo_disp . '>
            <a href="#" class="consulta_estoque" prod="' . $campos['codigo'] . '" >' . number_format($saldo_ref_disp,  2, ',', '.') . '</a></td>
