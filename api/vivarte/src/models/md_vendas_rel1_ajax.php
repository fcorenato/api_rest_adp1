...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $query1 = "
          select un.descricao as un_nome, pv.vend1, vend.nome_completo, pv.cliente_razao, pv.id as ped_num, pv.orc_data_emissao, pg.descricao as dec_pgto, pv.orc_split_pgto, pv.total_cimp, pv.frete_valor
    ...
    while ($campos = mysql_fetch_array($result_query1)) {
      ...
      if ($nome_unidade != $subtotal_rel and $subtotal_rel != "inicial") {
        $resultado_rel .= '
          <tr class="' . $bg_subtotal . ' tr_result">
              <td colspan="4">TOTAL ' . $subtotal_rel . '</td>
      ...
      if ($tipo_rel == 'analitico') {
        $resultado_rel .= '<tr class="tr_result">
            ...
            <td>' . $campos['orc_data_emissao'] . '</td>
            <td>' . $campos['dec_pgto'] . '</td>
            <td>' . $campos['orc_split_pgto'] . '</td>
