...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $pesquisa = mysql_query("SELECT u.codigo, u.usuario, u.nome_completo, u.status, u.senha, u.perfil, u.aprovador, u.analista_comerc, u.cod_vend, u.data_niver, u.instagram_pessoal, u.telefone, 
    un.codigo as un_codigo, un.descricao, un.cod_protheus, un.tabelas, un.orcamentos, un.armazens, un.cond_pgto_lib, un.split_pgto, un.fabricantes, un.orc_create  FROM `sys_usuarios` as u
    LEFT JOIN sys_unidades as un ON u.unidade_codigo = un.codigo
    WHERE usuario = '$post_usuario' AND un.status = 'A' AND u.status != 'I' ");
    ...
    if ($linhas == 0) {
        ...
        while ($dados = mysql_fetch_array($pesquisa)) {
            ...
            $orc_create = $dados["orc_create"];
            $usuario_data_niver = $dados["data_niver"];
            $usuario_instagram_pessoal = $dados["instagram_pessoal"];
            $usuario_telefone = $dados["telefone"];
        ...
        if (($status == 'A') and ($usuario != $senha)) {
            if ($senha != $post_pwd) {
                ...
                $_SESSION["orc_create"] = $orc_create;
                $_SESSION["usuario_data_niver"] = $usuario_data_niver;
                $_SESSION["usuario_instagram_pessoal"] = $usuario_instagram_pessoal;
                $_SESSION["usuario_telefone"] = $usuario_telefone;
    
                $resultado_login = 4; // login sucesso

                //registrando data login
                $ultimo_login = date("Y-m-d H:i:s");
                $pesquisa = mysql_query("UPDATE sys_usuarios SET ultimo_login = '$ultimo_login' WHERE codigo = $cd_usuario");
